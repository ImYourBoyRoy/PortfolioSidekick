...
import { useMarket } from '../context/SidekickContext';
...
export default function InsiderTab() {
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div className="glass-card" style={{ padding: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0 0', fontSize: '11px', color: 'var(--text-muted)', maxWidth: 560, lineHeight: 1.5 }}>
            ...
            New trades may not appear for up to {m.STOCK_ACT_MAX_LAG_DAYS} days after the transaction.
          </p>
          {m.congressSyncStatus?.label && (
            <p style={{ margin: '6px 0 0 0', fontSize: '10px', color: 'var(--text-secondary)', maxWidth: 560 }}>
              {m.congressSyncStatus.label}
              {m.congressSyncStatus.absoluteSynced ? (
                <span style={{ color: 'var(--text-muted)' }}> ({m.congressSyncStatus.absoluteSynced})</span>
              ) : null}
            </p>
          )}
        </div>
        <button
          onClick={() => m.loadCongressTrades(true)}
          disabled={m.congressLoading}
          className="btn-primary"
          style={{ padding: '10px 16px', fontSize: '11px', fontWeight: 800, borderRadius: 10, display: 'inline-flex', alignItems: 'center', gap: 6, opacity: m.congressLoading ? 0.6 : 1 }}
        >
          <RefreshCw className={m.congressLoading ? 'animate-spin' : ''} style={{ width: 13, height: 13 }} />
          {m.congressLoading ? 'Loading…' : 'Refresh'}
        </button>
      </div>

      {m.congressData?.disclaimer && (
        <div
          style={{
            fontSize: '10px',
        ...
        >
          <strong style={{ color: '#34d399' }}>Disclosure timing:</strong> {m.congressData.disclaimer}
        </div>
      )}

      {m.congressData?.warnings?.length > 0 && (
        <div className="glass-card" style={{ padding: 14, fontSize: '11px', color: '#fbbf24', border: '1px solid rgba(245,158,11,0.2)' }}>
          ...
          <div style={{ marginTop: 6, fontSize: '10px', color: 'var(--text-muted)' }}>
            {m.congressData.warnings.join(' ')}
          </div>
        </div>
      )}

      {m.congressLoading && !m.congressData && (
      ...
      {m.congressData?.error && m.congressData.trades?.length === 0 && (
        <div className="glass-card" style={{ padding: 28, textAlign: 'center', color: '#fbbf24', fontSize: '12px', border: '1px solid rgba(245,158,11,0.2)' }}>
          {m.congressData.error}
        </div>
      )}

      {m.congressData && m.congressData.trades?.length > 0 && (
        <div className="glass-card" style={{ padding: 18 }}>
          <h4 style={{ margin: '0 0 12px 0', fontSize: '12px', fontWeight: 900, color: '#34d399', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            Tracked officials · {m.congressData.total} recent filing{m.congressData.total === 1 ? '' : 's'}
          </h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {m.congressData.trades.map((trade) => {
              ...
              return (
                <button
                  key={trade.id}
                  onClick={() => trade.ptrLink && m.openNewsLink(trade.ptrLink)}
                  style={{
                    textAlign: 'left',
                ...
                >
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    ...
                    <span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>
                      Disclosed {m.formatCongressTradeDate(trade.disclosureDate)}
                      {trade.transactionDate ? ` · Traded ${m.formatCongressTradeDate(trade.transactionDate)}` : ''}
      ...
      {m.congressData && !m.congressData.error && m.congressData.trades?.length === 0 && !m.congressLoading && (
