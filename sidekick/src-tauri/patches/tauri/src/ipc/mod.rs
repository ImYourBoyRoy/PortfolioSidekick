...
/// Serialize command failures without a blanket `From<T> for InvokeError` impl.
///
/// Blanket `From` impls conflict with `time` 0.3.48+ on current Rust toolchains
/// (see time-rs/time#783). Plugin and app error enums should implement
/// [`Serialize`] to use the default conversion.
pub trait CommandErrorLike: Sized {
  /// Convert this command failure into an IPC [`InvokeError`].
  fn to_invoke_error(self) -> InvokeError;
}

impl<T: Serialize + Sized> CommandErrorLike for T {
  #[inline]
  fn to_invoke_error(self) -> InvokeError {
...
impl CommandErrorLike for InvokeError {
  #[inline(always)]
  fn to_invoke_error(self) -> InvokeError {
    self
  }
...
impl CommandErrorLike for crate::Error {
  #[inline(always)]
  fn to_invoke_error(self) -> InvokeError {
    InvokeError::from(self)
  }
}
...
impl<T: IpcResponse, E: CommandErrorLike> From<Result<T, E>> for InvokeResponse {
  #[inline]
  fn from(result: Result<T, E>) -> Self {
    match result {
      ...
      Err(err) => Self::Err(err.to_invoke_error()),
    }
  }
}
