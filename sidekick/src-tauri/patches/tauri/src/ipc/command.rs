...
pub mod private {
  use crate::{
    ipc::{CommandErrorLike, InvokeError, InvokeResolver, InvokeResponseBody, IpcResponse},
    Runtime,
  };
  ...
  // ===== Result<impl Serialize, impl CommandErrorLike> =====
  ...
  impl<T: IpcResponse, E: CommandErrorLike> ResultKind for Result<T, E> {}
  ...
  impl ResultTag {
    ...
    where
      ...
      E: CommandErrorLike,
    {
      resolver.respond(value.map_err(CommandErrorLike::to_invoke_error))
    }
    ...
    where
      ...
      E: CommandErrorLike,
    {
      Ok(value.map_err(CommandErrorLike::to_invoke_error)?.body()?)
    }
  }
  ...
  // ===== Future<Output = Result<impl Serialize, impl CommandErrorLike>> =====
  ...
  impl<T: IpcResponse, E: CommandErrorLike, F: Future<Output = Result<T, E>>> ResultFutureKind
    for F
  {
  }
  ...
  impl ResultFutureTag {
    ...
    {
      let response = value.await.map_err(CommandErrorLike::to_invoke_error)?;
      Ok(response.body()?)
    }
  }
}
