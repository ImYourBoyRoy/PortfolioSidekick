// ./sidekick/scripts/upgrade-ci-toolchain.mjs
/**
 * Audit and optionally refresh shared CI / desktop toolchain pins.
 *
 * Policy (see AGENTS.md):
 * - Node: sidekick/.node-version holds the major line; CI uses setup-node check-latest.
...
const repoRoot = resolve(sidekickRoot, '..');
...
function auditRustToolchain() {
  const path = resolve(sidekickRoot, 'src-tauri/rust-toolchain.toml');
...
function auditNodeVersionFile() {
  const path = resolve(sidekickRoot, '.node-version');
...
function auditPackageEngines() {
  const pkg = JSON.parse(readText(resolve(sidekickRoot, 'package.json')));
