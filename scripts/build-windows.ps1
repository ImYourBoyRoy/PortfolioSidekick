...
if ($nodeMajor -lt 26) {
    Write-Error "Node.js >= 26 is required (see sidekick/.node-version)."
    Exit 1
}
...
Push-Location (Join-Path $RepoRoot "sidekick")
$failed = $false
...
if ($tauriExit -eq 0) {
    $bundleDir = Join-Path $RepoRoot "sidekick\src-tauri\target\release\bundle"
    Write-Host "`n==========================================================" -ForegroundColor Green
    Write-Host "     [SUCCESS] TAURI BUILD COMPLETED!" -ForegroundColor Green
    Write-Host "==========================================================" -ForegroundColor Green
    $releaseDir = Join-Path $RepoRoot "sidekick\src-tauri\target\release"
