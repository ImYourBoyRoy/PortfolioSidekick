// ./frontend/src/serverless/congressTrades.js
...
async function establishSenateSession() {
  ...
  const landingHtml = await http.getText(`${SENATE_EFD}/search/home/`, { timeoutMs: 60000 });
  ...
  const agreeHtml = await http.postForm(
    ...
    { timeoutMs: 60000, headers: { Referer: `${SENATE_EFD}/search/home/` } }
...
export async function fetchCongressTrades(options = {}) {
  ...
  const payload = enrichCongressPayload(

    {
      ...
      error: errors.length && trades.length === 0 ? errors.join(' ') : errors.length ? errors.join(' ') : null,
